 ...
 * @param {Object} options { allowShortHand: Boolean } — 透传给 evaluateHand
 * @returns {Object} { score, handType, base, mult, chips, level, formula }
 */
export function scoreHand(cards, handLevels = {}, options = {}) {
  if (!cards || cards.length === 0) {
    return { score: 0, handType: null, base: 0, mult: 0, chips: 0, level: 0, formula: '' };
  }

  const handType = evaluateHand(cards, options);
